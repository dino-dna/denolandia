...
import * as scraper from 'module-scraper'

var wsService = require('./ws')

export async function init (app: Koa, config: denolandia.IServiceConfig): Promise<denolandia.IServices> {
  ...
  scraper.scrapeDatabase({
    poll: true,
    leadingEdge: !!(await client.query('select * from denolandia.modules')).rowCount
  })
  client.end(() => {})
